...
export default function Home() {
  ...
  const [fileType, setFileType] = useState<"image" | "csv" | "json">("image");
  ...
  async function submit(file?: File | Blob) {
    ...
    if (fileType === "image") {
      ...
      if (!isSupportedCSVType(file.type)) {
        return toast.error("Unsupported format. Only CSV files are supported.");
      }
      type = file.type;
    } else if (fileType === "json") {
      if (!isSupportedJSONType(file.type)) {
        return toast.error("Unsupported format. Only JSON files are supported.");
      }
      type = file.type;
    } else {
      return;
    ...
    if (file.size > 4.5 * 1024 * 1024) {
      return toast.error("File too large, maximum file size is 4.5MB.");
    }
  ...
  function handleFileTypeChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setFileType(e.target.value as "image" | "csv" | "json");
  }

  function handleCustomPromptChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
              ...
              <>
                ...
                <div className="mt-3 flex flex-col items-center gap-2">
                  <select
                    ...
                    className="rounded-full bg-gray-200 dark:bg-gray-800 px-4 py-2 text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-700"
                  >
                  ...
                  {fileType !== "image" && (
                    <textarea
                      ...
                      className="w-full rounded-md bg-gray-200 dark:bg-gray-800 px-4 py-2 text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-700"
                    />
                  )}
                </div>
